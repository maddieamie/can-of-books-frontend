...
export default class ToastAlert extends Component{
    render(){
        const { displayToast1, displayToast2, deleteBook, closeToast, deleted } = this.props;
...
{displayToast1 && (
    <Toast variant="danger" animation={false}>
      ...
      <button type="button" class="btn btn-warning btn-sm" onClick={deleteBook} id="toastdelete"> Delete Book</button>
      <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="toast" onClick={closeToast} > Close Message</button>
    </div>
          </Toast.Body>
